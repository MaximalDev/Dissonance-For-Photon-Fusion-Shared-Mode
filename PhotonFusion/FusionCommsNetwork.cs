...
namespace Dissonance.Integrations.PhotonFusion
{
        ...
        : BaseCommsNetwork<FusionServer, FusionClient, FusionPeer, Unit, Unit>, INetworkRunnerCallbacks
    {
        ...
        private bool IsShared => _runner != null && _runner.GameMode == GameMode.Shared;
        ...
        internal void SendToServer(ArraySegment<byte> packet, bool reliable)
        {
            if (Server != null)
            {
                _serverMessageQueue.Enqueue((new FusionPeer(_runner.LocalPlayer, true), CopyForLoopback(packet)));
                return;
            }
            ...
            if (IsShared)
            {
                ...
                {
                    RpcHandler.SendToSharedServer(this, packet, reliable);
                }
                return;
            }
        ...
        {
            var array = _byteArrayPool.Count == 0 ? new byte[1024] : _byteArrayPool.Dequeue();
            return packet.CopyToSegment(array);
        }

        internal void DeliverMessageToServer(byte[] data, PlayerRef source)
        {
            _serverMessageQueue.Enqueue((new FusionPeer(source, false), new ArraySegment<byte>(data)));
        }
